...
public class SharedPreferenceProvider extends ContentProvider {
    ...
    private MethodProcess methodQueryPid = new MethodProcess() {
        ...
        public Bundle process(@Nullable String arg, @Nullable Bundle extras) {
            ...
            bundle.putInt(SharedPreferenceUtil.KEY_VALUES, Process.myPid());
            return bundle;
        }
    };
    ...
    private MethodProcess methodContainKey = new MethodProcess() {
        ...
        public Bundle process(@Nullable String arg, @Nullable Bundle extras) {
            ...
            String key = extras.getString(OpEntry.KEY_KEY);
            SharedPreferences preferences = ctx.getSharedPreferences(arg, Context.MODE_PRIVATE);
            extras.putBoolean(SharedPreferenceUtil.KEY_VALUES, preferences.contains(key));
            return extras;
        }
    };
    private MethodProcess methodEditor = new MethodProcess() {
        ...
        public Bundle process(@Nullable String arg, @Nullable Bundle extras) {
            ...
            SharedPreferences preferences = ctx.getSharedPreferences(arg, Context.MODE_PRIVATE);
            ArrayList<Bundle> ops = extras.getParcelableArrayList(SharedPreferenceUtil.KEY_VALUES);
            if (ops == null) {
                ops = new ArrayList<>();
            }
            ...
            if (applyOrCommit == OpEntry.OP_TYPE_APPLY) {
                ...
                Bundle bundle = new Bundle();
                bundle.putBoolean(SharedPreferenceUtil.KEY_VALUES, res);
                return bundle;
            } else {
    ...
    public boolean onCreate() {
        processerMap.put(SharedPreferenceUtil.METHOD_QUERY_VALUE, methodQueryValues);
        processerMap.put(SharedPreferenceUtil.METHOD_CONTAIN_KEY, methodContainKey);
        processerMap.put(SharedPreferenceUtil.METHOD_EIDIT_VALUE, methodEditor);
        processerMap.put(SharedPreferenceUtil.METHOD_QUERY_PID, methodQueryPid);
        return true;
    }
