...
public class SharedPreferenceProxy implements SharedPreferences {
    ...
    public static SharedPreferences getSharedPreferences(@NonNull Context ctx, String preferName) {
        ...
        if (processFlag.get() == 0) {
            Bundle bundle = ctx.getContentResolver().call(SharedPreferenceUtil.URI, SharedPreferenceUtil.METHOD_QUERY_PID, "", null);
            int pid = 0;
            if (bundle != null) {
                pid = bundle.getInt(SharedPreferenceUtil.KEY_VALUES);
            }
    ...
    public boolean contains(String key) {
        ...
        try {
            Bundle res = ctx.getContentResolver().call(SharedPreferenceUtil.URI
                    , SharedPreferenceUtil.METHOD_CONTAIN_KEY
                    , preferName
                    , input);
            return res.getBoolean(SharedPreferenceUtil.KEY_VALUES);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    private OpEntry getResult(@NonNull OpEntry input) {
        try {
            Bundle res = ctx.getContentResolver().call(SharedPreferenceUtil.URI
                    , SharedPreferenceUtil.METHOD_QUERY_VALUE
                    , preferName
                    , input.getBundle());
    ...
    public class EditorImpl implements Editor {
        ...
        public boolean commit() {
            Bundle input = new Bundle();
            input.putParcelableArrayList(SharedPreferenceUtil.KEY_VALUES, convertBundleList());
            input.putInt(OpEntry.KEY_OP_TYPE, OpEntry.OP_TYPE_COMMIT);
            Bundle res = null;
            try {
                res = ctx.getContentResolver().call(SharedPreferenceUtil.URI, SharedPreferenceUtil.METHOD_EIDIT_VALUE, preferName, input);
            } catch (Exception e) {
                e.printStackTrace();
            }
            if (res == null) {
            ...
            return res.getBoolean(SharedPreferenceUtil.KEY_VALUES, false);
        }

        @Override
        public void apply() {
            Bundle intput = new Bundle();
            intput.putParcelableArrayList(SharedPreferenceUtil.KEY_VALUES, convertBundleList());
            intput.putInt(OpEntry.KEY_OP_TYPE, OpEntry.OP_TYPE_APPLY);
            try {
                ctx.getContentResolver().call(SharedPreferenceUtil.URI, SharedPreferenceUtil.METHOD_EIDIT_VALUE, preferName, intput);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
